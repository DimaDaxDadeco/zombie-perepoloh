...
export const CONFIG = {
  ...
  weapons: {
    ...
    // Огненный след: горит там, где герой пробежал. Единственное оружие, у
    ...
    firetrail: {
      name: 'Огненный след', emoji: '🔥', evolution: 'lavatrail',
      // «Кулдаун» тут — как часто роняется пятно, а не выстрел.
      cooldown: [0.28, 0.24, 0.2, 0.17, 0.14],
      // Широко — и это не щедрость, а геометрия, вскрытая замером. Зомби идут
      // не по следу, а срезают к текущей позиции героя, и узкая дорожка
      // остаётся в стороне: на радиусе 52 оружие давало 0 побед из 15, на 90 —
      // две, и только к 115 вышло на 11, то есть в середину разброса прочих.
      radius:   [115, 128, 142, 158, 178],
      burnDps:  [3, 4, 5, 6, 7],
      burnTime: 1.6,
      patchLife: 3.5,
      // Сплюснутость пятна. Не 0.55, как у паутины: та летит вперёд, и зомби
      // входят в неё сбоку, а след лежит ПОД героем — преследователь стоит от
      // него примерно в сорока пикселях, и при сильной сплюснутости пятно до
      // него по вертикали просто не достаёт. Замер это и показал: на прежних
      // числах след не убивал вообще никого.
      squash: 0.72,
    },
    ...
    lavatrail: {
      evolved: true,
      name: 'Лавовая дорожка', emoji: '🌋', about: 'След шире и горит дольше',
      cooldown: [0.11], radius: [230], burnDps: [9], burnTime: 2.2, patchLife: 4.5,
      squash: 0.72,
    },
    bubblestorm: {
      evolved: true,
