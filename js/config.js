...
export const CONFIG = {
  ...
  abilities: {
    ...
    batmobile: {
      ...
      // машина укладывает поперёк арены около четырёх волн — заметно виляет,
      // но не мельтешит.
      waveAmp: 80, waveLength: 260,
  ...
  weapons: {
    ...
    // Огненная дорожка: горит там, где герой пробежал. Единственное оружие, у
    // которого нет ни выстрела, ни цели — оно вообще не целится. Ребёнок
    // бегает и так, поэтому оружие награждает за то, что он и без того делает.
    firetrail: {
      ...
      // замкнуть кольцо. Ширина тут — та, с которой звено РОЖДАЕТСЯ; дальше
      // оно разгорается, см. grow.
      //
      // Откат — как часто роняется звено ленты, а не выстрел. Он подобран под
      // скорость бега: на 230 пикселях в секунду и шаге 0.07 звенья ложатся
      // через 16 пикселей при диаметре 22, то есть перекрываются и дорожка
      // выходит сплошной. Сделаете откат больше — лента порвётся на точки, и
      // зомби будут проходить между ними.
      cooldown: [0.07, 0.065, 0.06, 0.055, 0.05],
      radius:   [11, 12, 13, 14, 16],
      burnDps:  [4, 5, 7, 9, 12],
      burnTime: 1.6,
      patchLife: 5,
      // Переброска огня на соседа: радиус и во сколько раз слабее исходного.
      // Без неё дорожка бесполезна убегающему — наказывает ровно того, кто на
      // неё наступил, а толпа идёт следом по остывшим звеньям.
      // Во сколько раз звено расходится за свою жизнь: 11 пикселей под ногами
      // превращаются в лужу шестидесяти. Число подобрано замером, а не на
      // глаз: 3.5 даёт 3 победы из 15, 4 — семь, 4.5 — одиннадцать, вровень с
      // пузырями и торнадо. Выше не идём, 5 даёт девять — это уже шум.
      grow: 4.5,
      spreadRange: 34,
      spreadFactor: 0.6,
      // Почти круглые звенья: у широкой лужи сплюснутость читалась как «лежит
      // на земле», а тонкую ленту она рвёт по вертикали.
      squash: 0.85,
    },
    ...
    bubbles: {
      ...
      holdTime: [1.4, 1.6, 1.8, 2.0, 2.4],
      speed: 250,
      radius: 26,
    },

    // Торнадо: вихрь бродит сам по себе и тащит за собой зомби. От Портала
    ...
    tornado: {
      ...
      cooldown: [4.5, 4, 3.5, 3, 2.6],
      // Меньше, чем было (110–175): тот перекрывал четверть арены и читался
      // как погода, а не как оружие. Силу вернули не размером, а частотой
      // ударов и уроном — по замеру вышло ровно столько же, 11 побед из 15.
      radius:   [80, 90, 100, 112, 126],
      damage:   [2, 2, 3, 3, 4],
      pull: 260,
      life: 7,
      speed: 90,
      tickTime: 0.25,  // как часто вихрь бьёт тех, кто внутри
    },
  ...
  pets: {
    ...
    // Спасённый из клетки. В магазине его нет и быть не может: он не товар,
    // а награда за главу, и живёт только до конца раунда.
    //
    // Не кусает, а ОТПИХИВАЕТ. Урон почти номинальный: друг должен помогать,
    // а не воевать — иначе спасённая толпа проходит главу за ребёнка. Отброс,
    // наоборот, щедрый: он закрывает ровно ту дыру, которую пятилетний
    // закрыть не умеет, — тех, кто подобрался сзади.
    friend: {
      name: 'Друг', radius: 13,
      followDistance: 52, followOffset: 20, personalSpace: 32,
      maxSpeed: 290, accel: 1400, friction: 0.02,
      wanderSpeed: 2.4, wanderAmount: 12,
      chaseRadius: 200, leash: 240,
      damage: 1, cooldown: 1.6, biteRange: 8, force: 320,
      look: { skin: '#ffcc80', clothes: '#7e57c2', hair: '#5d4037', cape: null },
    },
    drone: {
  ...
  // Сюжетные путешествия.
  //
  // Путешествие — история со своей картой, своим прогрессом и своим финалом.
  // Список, а не одна запись: третье добавляется одной записью и нулём кода.
  //
  // ГЛАВНОЕ ПРАВИЛО: id глав уникальны по ВСЕЙ игре, не по путешествию.
  // Сохранение хранит плоский список пройденных id, а прогресс путешествия —
  // это пересечение с его главами. Благодаря этому у второго путешествия нет
  // никакой миграции: у ребёнка лежат ch1…ch12, они принадлежат первому, и он
  // видит ровно то же, что видел. Цена — старые id неприкосновенны, а новое
  // путешествие берёт свой префикс. Проверяется тестом.
  //
  // needs — какое путешествие надо пройти, чтобы это открылось. Закрытое не
  // показывается нигде: карта с замком — это обещание, на которое нельзя
  // нажать, а нажимать ребёнок будет.
  //
  // reward — чем награждает глава. У первого путешествия это страница
  // альбома, у второго будет своё, и экран победы не должен знать про
  // страницы.
  //
  // Кадры историй живут здесь, а не в game.js: это контент, а не логика.
  // Одна фраза на кадр — длиннее ребёнок не дослушает, да и Speech всё равно
  // не умеет говорить две подряд.
  //
  // level — это номер раунда, который получит Round. В проекте round не
  // «номер уровня», а универсальная ось прогрессии: от неё считаются виды
  // зомби (fromRound идёт ровно с 1 по 12), ступенька HP, темп спавнера и HP
  // босса. Двенадцать глав ложатся на эту ось один в один, поэтому кривая
  // сложности бесплатно совпадает с уже отлаженной кривой обычной игры.
  // Замер показал, что выше двенадцатого проходимость обрывается (8 побед из
  // 20 против 18), поэтому и второе путешествие идёт по той же оси, а
  // «сложнее» дают задачи, а не номер.
  //
  ...
  journeys: [
    {
      ...
      title: 'Верни альбом',
      icon: 'ui-map',
      needs: null,                 // первое, открыто всегда
      medal: 'pagesBack',
      reward: {
        icon: 'ui-page',
        word: 'страниц',
        line: 'СТРАНИЦА ВЕРНУЛАСЬ!',
        voice: 'Ура! Страница вернулась в альбом!',
      },
      intro: [
    ...
    // Второе путешествие. Открыто СРАЗУ, наравне с первым.
    //
    // Сначала оно пряталось до финала первого — чтобы новая карта была
    // событием. От этого отказались: двенадцать глав до неё превращались в
    // забор, а на карте у ребёнка и так есть понятный выбор, куда идти.
    // Уровни у обоих путешествий одни и те же (1…12), так что вторым можно
    // начинать без всякой форы.
    //
    ...
    // содержанием, а не номером.
    {
      ...
      // Завязка: играется, когда ребёнок заходит сюда впервые.
      intro: [
  ...
  goals: {
    ...
    // --- Задачи со своими объектами мира ---
    //
    // У всех четырёх босса нет, поэтому все четыре уплотняют волны тем же
    // способом, что и «продержись»: иначе глава ощущается передышкой.

    // Освободить друзей из клеток: подбежать и постоять рядом.
    rescue: {
    ...
    // Отнести ношу на светлое место.
    delivery: {
      icon: 'ui-box', count: 6,
      // С ношей заметно медленнее, но убежать всё ещё можно — тот же принцип,
      // что у заморозки Морозко (0.55 читается как «влип», 0.78 как «тяжело»).
      carryFactor: 0.78,
      loadRadius: 16, zoneRadius: 46,
      minRunDistance: 300,  // ноша не появляется рядом с местом доставки
      intervalFactor: 0.9, batchBonus: 1,
    },

    // Беречь костёр. Счёт в СЕКУНДАХ ГОРЕНИЯ, а не в секундах раунда: пока
    // костёр потушен, счётчик замирает, и задача звучит как «набери», а не
    // как «успей».
    campfire: {
      icon: 'ui-fire', count: 40,
      radius: 34, reach: 8,
      douseRate: 0.16,      // насколько быстро тушит один зомби
      relightRate: 0.14,    // и насколько сам разгорается без них
      maxDousers: 4,        // потолок: без него толпа гасит костёр мгновенно
      burnDps: 1, burnTime: 1.2,
      // Часть толпы остаётся на герое: уйди к костру все, глава превратилась
      // бы в статичную оборону, а ребёнок умеет только бегать.
      lureShare: 0.7,
      // Волны НЕ уплотняются добавкой в пачку, в отличие от прочих задач без
      // босса. Замер объяснил почему: с ней бот проигрывал семь раз из
      // двенадцати, и проигрывал не костром, а собственной гибелью — задача
      // и так держит героя на одном месте, у толпы. Без добавки выходит
      // одиннадцать из двенадцати, верх коридора.
      intervalFactor: 0.85, batchBonus: 0,
    },

    // Догнать воришку.
    thief: {
      icon: 'ui-bag', count: 5,
      // Доля от скорости САМОГО МЕДЛЕННОГО героя. Меньше единицы: воришка
      // чуть медленнее, и упорный ребёнок догоняет его сам, не дожидаясь
      // передышки.
      //
      // Сначала он был БЫСТРЕЕ героя (1.06) — чтобы погоня не кончалась за
      // пять секунд у идеального бота. Замер ботом, который бегает как
      // пятилетний (реакция раз в полсекунды, промах по углу, паузы), показал
      // цену: восемь побед из двадцати вместо шестнадцати. Причём проигрывал
      // он не воришкам, а толпе — в этой задаче бежать приходится НА зомби, а
      // не от них, и лишние секунды погони оплачиваются сердечками.
      speedFactor: 0.95,
      // Ловят его РОВНО на передышке, и в этом вся задача: за секунду стоя
      // герой отыгрывает больше, чем воришка выигрывает за пять секунд бега.
      // Отсюда же и проходимость любым оружием — исход решают ноги, а не
      // ствол.
      runTime: 5, restTime: 1,
      leash: 520,
      // Насколько близко к краю он готов подбежать и на сколько смотрит
      ...
      // Волны РЕЖЕ, чем в обычном раунде, — единственная задача, где так.
      // Причина не в темпе, а в том, что здесь нельзя убегать: ребёнок бежит
      // НА зомби, и каждая лишняя секунда погони оплачивается сердечками.
      // Замер «детским» ботом: 1.05 давало 19–21 победу из тридцати, 1.3 даёт
      // 25–27, то есть коридор. Дальше не смягчаем: с уменьшенной пачкой
      // выходит 30 из 30, а это уже не задача.
      intervalFactor: 1.3, batchBonus: 0,
    },
  },
  ...
  // Как показывать мир — спрашивается при новой игре, между сложностью и
  // выбором героя. Подписи для взрослого и для озвучки; ребёнок выбирает по
  // картинке: плоский круг против гранёного кубика.
  views: [
    { id: 'flat', name: 'Обычная', about: 'Вид сверху, как всегда', icon: 'ui-flat' },
    { id: 'solid', name: 'Объёмная', about: 'Всё как настоящее', icon: 'ui-cube' },
  ],

  // Объёмный режим (см. docs/render3d.md). На симуляцию не влияет ничем:
  // это только про то, как показан тот же самый мир.
  render3d: {
    fov: 55,               // вертикальный угол обзора камеры, градусы
    near: 10,
    far: 8000,

    // Камера считается не в пикселях, а в долях от МЕНЬШЕЙ стороны арены:
    // окно бывает и узким телефоном, и широким монитором, а кадр должен
    // охватывать поле одинаково в обоих.
    heightFactor: 0.85,    // насколько камера поднята над полем
    distanceFactor: 0.55,  // насколько отодвинута назад, за спину героя
    // Пределы для колеса и вертикального движения мыши: выше — вид почти
    // сверху, ниже — настоящий взгляд из-за плеча.
    minHeightFactor: 0.30,
    maxHeightFactor: 1.80,

    followLerp: 7,         // скорость, с которой камера догоняет героя
    mouseYawSpeed: 0.007,  // радиан на пиксель горизонтального движения мыши
    mousePitchSpeed: 0.004,
    wheelStep: 0.09,

    // Свет. Направленный даёт тени и объём, полусферный подсвечивает
    // изнанку — без него всё, что не смотрит на солнце, становится чёрным.
    light: {
      sunColor: '#fff6e0',
      sunIntensity: 1.9,
      // Направление на солнце, в долях от размера арены. Слева-сверху и
      // немного СПЕРЕДИ: при стартовом угле камеры фигурки повёрнуты к ней
      // лицом, и солнце из-за поля оставляло бы их силуэтами. Немного —
      // потому что при солнце ровно за спиной зрителя тень прячется под
      // фигуркой, и персонаж перестаёт стоять на земле.
      sunAt: { x: -0.55, y: 1.5, z: 0.4 },
      skyColor: '#dff0ff',
      groundColor: '#8a8270',
      ambientIntensity: 1.45,
      shadowSize: 2048,
      // Объём тени считается отдельно от размера пола: пол намеренно уходит
      // далеко за арену, и растянутая на него карта теней стала бы мылом.
      shadowSpan: 0.9,
    },

    // Эффекты оружия и способностей (см. docs/render3d.md).
    //
    // maxStickers — потолок наклеек в кадре. Их источники наперечёт: ствол в
    // руке у каждого игрока, пузыри пойманных, ярость и полоса здоровья
    // босса. Сотни хватает с большим запасом, а потолок держит пул от роста
    // на случай, если новое оружие начнёт сыпать их пачками.
    maxStickers: 120,
    // Сторона холста наземных эффектов. Он грузится в видеопамять только
    // когда на нём что-то есть, поэтому можно позволить себе разрешение:
    // огненная дорожка должна выглядеть огнём, а не пятном.
    groundDecalSize: 1024,
    // Появление босса: сколько эффектов разом и в каком разрешении. Их почти
    // всегда один — босс выходит по одному, — но два холста дешевле, чем
    // пропавший эффект у второго.
    maxEntrances: 2,
    entranceSize: 512,

    // Дальние фигурки. Честная перспектива делает зомби у дальнего края
    // вдвое мельче ближнего — в плоской игре такого не было вовсе, и для
    // пятилетнего это потеря, а не приобретение: врага надо ВИДЕТЬ.
    // Компенсация частичная: единица убила бы ощущение глубины, ноль —
    // читаемость. Осознанный размен в пользу читаемости.
    distanceCompensation: 0.45,
    maxDistanceBoost: 1.8,

    // Ночь. В плоской игре это тёмная заливка поверх мира и светлый круг у
    // героя; в объёме то же самое делается честно — гасим солнце и вешаем
    // герою фонарик. Радиус круга берётся из CONFIG.specialRounds.night,
    // чтобы свет и геймплейная «зона видимости» не разошлись.
    night: {
      // Ночь ПРИГЛУШАЕТ, а не гасит. Три множителя разом (солнце, рассеянный
      // свет и потемневшая земля) легко дают чёрный экран, а он в пять лет
      // страшен по-настоящему — про это же говорит комментарий к
      // specialRounds.night: зомби вне круга обязан оставаться узнаваемым
      // силуэтом, никто не выпрыгивает из пустоты.
      sunFactor: 0.42,       // во сколько раз тусклее солнце
      ambientFactor: 0.55,
      sky: '#0e1230',
      // Ночной свет ещё и СИНИЙ, а не просто слабый. Без смены оттенка песок
      // пляжа и сено фермы просто темнеют до бурого, и ночь читается как
      // мутный день. Синева — то же решение, что и тёмно-синий тинт плоской
      // версии: чёрный экран в пять лет страшен по-настоящему.
      skyColor: '#4a5fa8',
      groundColor: '#10162e',
      // Насколько цвет земли уводится в ночное небо. Число выше, чем
      // подсказывает глаз: Three смешивает цвета в линейном пространстве, и
      // «половина пути к тёмному» там выглядит гораздо светлее половины.
      // Не единица: пол обязан остаться отличимым от неба, иначе горизонт
      // пропадает вовсе.
      groundMix: 0.72,
      lampColor: '#ffe9b0',
      lampIntensity: 9,
      // Затухание фонарика. Пологое: круг света должен иметь мягкий край, а
      // при физически честном затухании он превращается в яркое пятно под
      // ногами и темноту в шаге от героя.
      lampDecay: 1.15,
    },

    // Бортик по краю поля. Без него в объёме непонятно, где мир кончается:
    // трава просто продолжается и тает в тумане, и ребёнок упирается в
    // невидимую стену. Низкий намеренно — бортик обязан обозначать границу,
    // а не прятать за собой зомби, которые к ней прижались.
    borderHeight: 16,
    borderThickness: 9,

    // Пол сильно больше арены: при взгляде из-за плеча дальний край поля
    // попадает в кадр, и обрыв земли с небом под ним читается как поломка.
    // Лишнее прячет туман — см. applyTheme.
    groundOverscan: 4,
  },

  themes: [
    { id: 'yard', name: 'Двор',   ground: '#7ec850', accent: '#5aa63c', deco: 'flowers', sky: '#bfe4ff' },
    { id: 'park', name: 'Парк',   ground: '#5aa956', accent: '#3d8a44', deco: 'trees', sky: '#a9dcf5' },
    { id: 'beach', name: 'Пляж',   ground: '#f0d9a0', accent: '#d9bd7d', deco: 'beach', sky: '#ffe4b0' },
    { id: 'space', name: 'Космос', ground: '#2b2d4a', accent: '#3d4066', deco: 'space', sky: '#0d0f22' },
    ...
    { id: 'cave', name: 'Пещера', ground: '#5a5468', accent: '#4a4459', deco: 'crystals', sky: '#332f42' },
    ...
    { id: 'rink', name: 'Каток',  ground: '#c6dcee', accent: '#aecbe2', deco: 'ice', sky: '#e8f4ff' },
    // Ферма отличается от Пляжа насыщенностью, а не оттенком, — смотреть их
    // надо рядом, а не по очереди.
    { id: 'farm', name: 'Ферма',  ground: '#c9a23c', accent: '#b08b2f', deco: 'hay', sky: '#ffdfa0' },
  ],
