...
export const CONFIG = {
  ...
  abilities: {
    ...
    portal: {
      ...
      damage: 12,       // столько получает дошедший: толстяк на поздних раундах выживет
      // Ключевое число. Портал тянет зомби НА героя, и если засасывать их
      // вплотную, они успевают куснуть по дороге: касание случается на
      // 20 + 21 = 41 px. Затягиваем заметно раньше этого — иначе способность
      // не спасает, а сама подтаскивает опасность.
      grip: 105,
    },
    turbo: {
