...
export class Player {
  constructor(x, y, {
    ...
    this.bitesTaken = 0;
    // Ноша: множитель скорости, пока герой что-то несёт. Обычным полем, а НЕ
    // опцией конструктора — деструктуризация выше разбирает фиксированный
    // список и молча роняет незнакомое, а ошибка выглядит как «ноша не
    // замедляет».
    this.carryFactor = 1;
  ...
  //
  // Ноша идёт ОТДЕЛЬНЫМ множителем, а не через chill(). Соблазн был: вонь
  // Хэнки замедляет зомби ровно так, каждый кадр. Но Player.chill присваивает
  // chillFactor БЕЗУСЛОВНО — побеждает последний вызов, — и ноша с её мягкими
  // 0.78 каждый кадр перетирала бы заморозку Морозко (0.55). Взял ношу —
  // вылечился от заморозки. Такое не покажет ни один тест.
  get speedFactor() {
    return (this.chillTimer > 0 ? this.chillFactor : 1) * this.carryFactor;
  }
